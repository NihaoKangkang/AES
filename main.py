...
if __name__ == "__main__":
    ...
    # inputKey = input(
    #     'Input 128 bits secret key in hex (like \'0123456789abcdef0123456789abcdef\', input nothing to abort): ')
    ...
    try:
        if inputData and len(inputKey) in [32, 48, 64]:
            ...
            if encodeFlag == 'y' or encodeFlag == 'Y' or encodeFlag == '':
                result = aes_encode(inputData, int(inputKey, 16))
                print('DES encode result: ', result.hex() if isinstance(result, bytes) else result)
            else:
                print('DES decode result: ', aes_decode(inputData, int(inputKey, 16)))
        else:
